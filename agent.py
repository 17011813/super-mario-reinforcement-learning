...
#체크포인트 받을라면 import os 해줘야 경로 받아올 수 있다
import os
...
class DQNAgent:
    ...
    def save_model(self):
        ...
        self.saver.save(sess=self.session, save_path='./models/model.ckpt', global_step=self.step)
        tf.io.write_graph(self.session.graph_def, '.', 'graph.pb', as_text=False)
...
#tf.io.write_graph(sess.graph_def, '.', 'graph.pbtxt', as_text=True)
